...
{
public:
	aiEvents(aiPerson* A,std::vector<aiPerson*>& aiLists, aiTime timeStarted);

	aiPerson* personA;
	ai_event eventType;
	aiTime timeStarted;
	aiTime estimatedLength;
	aiTime timeEnded;
...
public:
	...
	aiEvents* interruptiveEvent;
	aiPerson(double x, double y,std::string name);

	std::vector<Relation*> generateRelation(std::vector<aiPerson*>& npc);
	void updateBuff();
	void generateEvent(std::vector<aiPerson*>& npc,aiTime time);
	bool needToBeAltered(aiTime time);
	void generateNewEvent(aiTime time);
};
